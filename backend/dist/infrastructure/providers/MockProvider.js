...
export class MockProvider {
    ...
    async searchByEmail(email) {
        ...
        const result = this.mockData.find(person => person.email.toLowerCase() === email.toLowerCase());
        if (!result) {
            ...
            return {
                name,
                email,
                company: domain.split('.')[0].replace(/\b\w/g, l => l.toUpperCase()),
    ...
    async searchByCompany(companyName) {
        ...
        const results = this.mockData.filter(person => person.company.toLowerCase().includes(companyName.toLowerCase()));
        if (results.length === 0) {
    ...
    async searchByName(name) {
        ...
        const results = this.mockData.filter(person => person.name.toLowerCase().includes(name.toLowerCase()));
        return results;
    }
    delay(ms) {
